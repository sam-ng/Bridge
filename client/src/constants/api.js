export const SERVER_URL = `${process.env.REACT_APP_SERVER_SCHEME}${process.env.REACT_APP_SERVER_DOMAIN}:${process.env.REACT_APP_SERVER_PORT}`;
...
export const nonPrintableKeys = Object.keys(keys).reduce((acc, category) => {
  return [...acc, ...keys[category]];
}, []);

export const isKeyPrintable = (e) => {
  return nonPrintableKeys.indexOf(e.key) === -1;
};
